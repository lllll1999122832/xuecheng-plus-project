package tang.orders.service.impl;
...
import tang.orders.config.PayNotifyConfig;
import tang.orders.mapper.XcOrdersGoodsMapper;
import tang.orders.mapper.XcOrdersMapper;
import tang.orders.mapper.XcPayRecordMapper;
...
import tang.orders.model.po.XcOrders;
import tang.orders.model.po.XcOrdersGoods;
...
public class OrderServiceImpl implements OrderService {
    ...
    public void notifyPayResult(MqMessage mqMessage) {
        ...
        //创建发送消息的对象
        Message messageMq = MessageBuilder.withBody(message.getBytes(StandardCharsets.UTF_8))
                .setDeliveryMode(MessageDeliveryMode.PERSISTENT)//将消息设置为持久化
                .build();
        //需要指定全局消息id,不重复
        Long id = mqMessage.getId();
        CorrelationData correlationData = new CorrelationData(id.toString());//使用它可以指定回调方法
        correlationData.getFuture().addCallback(result->{
            ...
            //发生异常
            log.debug("发送消息异常:{}",message);
        });
        //发送消息
        rabbitTemplate.convertAndSend(
                PayNotifyConfig.PAYNOTIFY_EXCHANGE_FANOUT, //交换机
                null,//广播,不用key
                messageMq //消息
                ,correlationData //接受失败或者成功的逻辑处理
        );
    }
